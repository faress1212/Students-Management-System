...
function openGradeModal() {
    ...
    var sel = form.elements['studentId'];
...
function saveGrade() {
    try {
        ...
        var subj = form.elements['subject'].value.trim();
        var sc = +form.elements['score'].value;
        var sem = form.elements['semester'].value;
...
function toast(msg, type) {
    type = type || 'info';
    var colors = { success: '#059669', error: '#e74c3c', info: '#2563eb', warning: '#d97706' };
    var t = document.createElement('div');
    t.style.cssText = 'background:' + (colors[type] || colors.info) + ';color:#fff;padding:12px 18px;border-radius:8px;margin-top:8px;font-size:.9rem;box-shadow:0 4px 12px rgba(0,0,0,.2);animation:fadeIn .2s ease;max-width:320px;word-break:break-word;';
    t.textContent = msg;
